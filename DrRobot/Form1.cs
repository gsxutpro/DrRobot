...
namespace DrRobot
{
    public partial class Form1 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            ArduinoCommandBuilder cmd = new ArduinoCommandBuilder(type, ParameterType.Int32);
            cmd.AddParameter(Int32.Parse(tbParam1.Text));

            byte[] res = cmd.GetByteCommand();
            richTextBox1.AppendText(BytesToString(res));
            ArduinoSerialCommandBroker snd = new ArduinoSerialCommandBroker(comboBox1.Text);
            listBox1.Items.Insert(0,snd.SendCommand(res));
        }
        ...
        {
            Properties.Settings.Default.Default_port = comboBox1.Text;
        }

        private void button4_Click(object sender, EventArgs e)
        {
            Servo servo1 = new Servo(1, 7);
            SharpGP2Y0A02YK ir = new SharpGP2Y0A02YK(0);
            Dictionary<int, double> dist = new Dictionary<int, double>();
            for (int i = 0; i <= 180; i += 5)
            {
                servo1.SetAngle(i);
                dist.Add(i, ir.GetDistance());
            }
            DrawInPolar(pictureBox1.CreateGraphics(), dist);
        }
